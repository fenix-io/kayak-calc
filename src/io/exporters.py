...
def _write_csv_metadata(file, metadata: Dict[str, Any]):
    ...
    file.write(f"# Kayak Calculation Tool - Data Export\n")
    file.write("#\n")
...
def export_stability_metrics(
    ...
    meta = {
        ...
        'CG Position': f'LCG={metrics.cg_position[0]:.3f}, VCG={metrics.cg_position[1]:.3f}, TCG={metrics.cg_position[2]:.3f} m',
    }
    if metadata:
        meta.update(metadata)
    ...
    with open(filepath, 'w', newline='') as f:
        ...
        writer.writerow(['Angle of Maximum GZ', f'{metrics.angle_of_max_gz:.{precision}f}', 'degrees'])
        
        # Range of positive stability
        if not np.isnan(metrics.range_of_positive_stability[0]):
            writer.writerow(['Range Positive Stability (Min)', f'{metrics.range_of_positive_stability[0]:.{precision}f}', 'degrees'])
            writer.writerow(['Range Positive Stability (Max)', f'{metrics.range_of_positive_stability[1]:.{precision}f}', 'degrees'])
        else:
        ...
        if not np.isnan(metrics.angle_of_vanishing_stability):
            writer.writerow(['Angle of Vanishing Stability', f'{metrics.angle_of_vanishing_stability:.{precision}f}', 'degrees'])
        else:
        ...
        if metrics.area_under_curve is not None and not np.isnan(metrics.area_under_curve):
            writer.writerow(['Dynamic Stability (Area Under Curve)', f'{metrics.area_under_curve:.{precision}f}', 'm·rad'])
        else:
...
def export_cross_sections(
    ...
    with open(filepath, 'w', newline='') as f:
        ...
        # Header
        writer.writerow(['Station_m', 'Area_m2', 'Centroid_Y_m', 'Centroid_Z_m', 'Waterline_Beam_m'])
        
        # Data rows
        for station in stations:
            try:
                profile = hull.get_profile(station)
                props = calculate_cross_section_properties(
                    profile,
...
def export_waterline_comparison(
    ...
    with open(filepath, 'w', newline='') as f:
        ...
        for wl_z in sorted(results.keys()):
            ...
            writer.writerow([
                f'{wl_z:.{precision}f}',
                f'{metrics.gm_estimate:.{precision}f}' if metrics.gm_estimate is not None else 'N/A',
                f'{metrics.max_gz:.{precision}f}',
                f'{metrics.angle_of_max_gz:.{precision}f}',
                f'{metrics.angle_of_vanishing_stability:.{precision}f}' if not np.isnan(metrics.angle_of_vanishing_stability) else 'N/A',
                f'{metrics.range_of_positive_stability[0]:.{precision}f}' if not np.isnan(metrics.range_of_positive_stability[0]) else 'N/A',
                f'{metrics.range_of_positive_stability[1]:.{precision}f}' if not np.isnan(metrics.range_of_positive_stability[1]) else 'N/A',
...
def format_stability_summary(
    ...
    if not np.isnan(metrics.range_of_positive_stability[0]):
        lines.append(f"- Range of Positive Stability: {metrics.range_of_positive_stability[0]:.1f}° to {metrics.range_of_positive_stability[1]:.1f}°")
    else:
...
def generate_stability_report(
    ...
    with open(filepath, 'w') as f:
        ...
        f.write(f"- CG Position: LCG={curve.cg.lcg:.{precision}f} m, VCG={curve.cg.vcg:.{precision}f} m, TCG={curve.cg.tcg:.{precision}f} m\n")
        f.write(f"- Total Mass: {curve.cg.total_mass:.1f} kg\n")
        f.write(f"- Heel Angle Range: {curve.heel_angles[0]:.1f}° to {curve.heel_angles[-1]:.1f}°\n")
...
def generate_complete_report(
    ...
    with open(filepath, 'w') as f:
        ...
        # Metadata
        f.write(f"**Generated:** {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}  \n")
        f.write(f"**Kayak Calculation Tool**  \n")
        if metadata and 'name' in metadata:
            f.write(f"**Hull Name:** {metadata['name']}  \n")
        ...
        stations = hull.get_stations()
        f.write(f"- Station Range: {stations[0]:.{precision}f} m to {stations[-1]:.{precision}f} m\n")
        f.write("\n")
