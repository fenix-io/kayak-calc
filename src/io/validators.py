...
def validate_metadata(metadata: Dict[str, Any]) -> Tuple[bool, List[str]]:
    ...
    if 'water_density' in metadata and metadata['water_density'] is not None:
        try:
            ...
            errors.append(f"Water density must be a number, got {type(metadata['water_density']).__name__}")
...
def validate_point_data(point_dict: Dict[str, Any], point_index: Optional[int] = None) -> Tuple[bool, List[str]]:
    """
...
def validate_profile_data(profile_dict: Dict[str, Any], profile_index: Optional[int] = None) -> Tuple[bool, List[str]]:
    """
    ...
    try:
        ...
        errors.append(
            f"{prefix}Station must be numeric, "
            f"got {type(profile_dict['station']).__name__}"
        )
        return (False, errors)
    ...
    if len(points) < MIN_POINTS_PER_PROFILE:
        errors.append(
            f"{prefix}Must have at least {MIN_POINTS_PER_PROFILE} points, "
            f"got {len(points)}"
        )
...
def validate_hull_data(hull_data: Dict[str, Any]) -> Tuple[bool, List[str]]:
    ...
    Example:
        >>> hull = {
        ...     'metadata': {'units': 'm', 'coordinate_system': 'centerline_origin', 'water_density': 1025.0},
        ...     'profiles': [
        ...         {'station': 0.0, 'points': [{'x': 0.0, 'y': 0.0, 'z': 1.0}, {'x': 0.0, 'y': 1.0, 'z': 0.0}, {'x': 0.0, 'y': -1.0, 'z': 0.0}]},
        ...         {'station': 1.0, 'points': [{'x': 1.0, 'y': 0.0, 'z': 1.0}, {'x': 1.0, 'y': 1.0, 'z': 0.0}, {'x': 1.0, 'y': -1.0, 'z': 0.0}]}
        ...     ]
        ... }
    ...
    if len(profiles) < MIN_PROFILES_PER_HULL:
        errors.append(
            f"Hull must have at least {MIN_PROFILES_PER_HULL} profiles, "
            f"got {len(profiles)}"
        )
    ...
    if len(stations) > 0:
        ...
        if len(unique_stations) < len(stations):
            errors.append(
                f"Duplicate stations found. Each profile must have a unique station value."
            )
...
def validate_csv_data(
    ...
    if arr.shape[1] != expected_cols:
        errors.append(
            f"Expected {expected_cols} columns {columns}, "
            f"got {arr.shape[1]} columns"
        )
...
def validate_symmetry(points: List[Dict[str, float]], tolerance: float = 1e-6) -> Tuple[bool, List[str]]:
    """
