...
def interpolate_multiple_profiles(
    ...
    for target in target_stations:
        ...
        interpolated = interpolate_longitudinal(
            profile_before, profile_after, target, num_points
        )
...
def interpolate_to_apex(
    ...
    Example:
        ...
        3
    """
    # Determine if we're going forward (bow) or backward (stern)
    if apex_point.x < profile.station:
        direction = -1  # Going toward stern
    elif apex_point.x > profile.station:
        direction = 1   # Going toward bow
    else:
        raise ValueError(
    ...
    stations = np.linspace(
        profile.station,
        apex_point.x,
        num_intermediate_stations + 2  # +2 for start and end
    )[1:-1]  # Exclude start (original profile) and end (apex point)
    ...
    for station in stations:
        ...
        points_in_this_profile = max(
            int(num_points_per_profile * (1 - t * 0.5)),
            3  # Minimum 3 points
        )
        ...
        station_points = [
            Point3D(station, y, z)
            for y, z in zip(y_sampled, z_sampled)
        ]
...
def create_symmetric_profile(
    ...
    for point in starboard_points:
        ...
        if not np.isclose(point.x, station):
            raise ValueError(
                f"Point x={point.x} doesn't match station {station}"
            )
...
def resample_profile_uniform_y(
    profile: Profile,
    num_points: int
) -> Profile:
    """
...
def resample_profile_uniform_arc(
    profile: Profile,
    num_points: int
) -> Profile:
    """
