...
def apply_heel_to_profile(
    ...
    heeled_points = [
        apply_heel(point, heel_angle, reference_point)
...
def apply_heel_to_hull(
    ...
    heeled_hull = KayakHull(
        origin=apply_heel(hull.origin, heel_angle, reference_point)
    )
...
def apply_trim_to_profile(
    ...
    trimmed_points = [
        apply_trim(point, trim_angle, reference_point)
        for point in profile.points
    ]
    ...
    if trimmed_points:
        ...
        adjusted_points = [
            Point3D(new_station, p.y, p.z) for p in trimmed_points
        ]
    else:
...
def apply_trim_to_hull(
    ...
    trimmed_hull = KayakHull(
        origin=apply_trim(hull.origin, trim_angle, reference_point)
    )
...
def find_profile_waterline_intersection(
    profile: Profile,
    waterline: Waterline
) -> List[Point3D]:
    """
...
def calculate_submerged_area(
    ...
    area = 0.5 * np.abs(
        np.sum(y_coords * (np.roll(z_coords, -1) - np.roll(z_coords, 1)))
    )
...
def calculate_waterplane_area(
    ...
    Returns:
        Waterplane area
    """
    from .interpolation import interpolate_multiple_profiles
    ...
    all_profiles = list(hull.profiles.values())
...
def transform_to_body_coordinates(
    point: Point3D,
    hull_orientation: Tuple[float, float, float] = (0, 0, 0)
) -> Point3D:
    """
