...
def plot_profile(
    ...
    **kwargs,
    ...
    if abs(heel_angle) > 1e-6:
        transformed_points = [apply_heel(pt, heel_angle) for pt in profile.points]
    else:
    ...
    if show_submerged:
        ...
        if len(submerged_points) >= 3:
            ...
            min(pt[0] for pt in submerged_points)
            max(pt[0] for pt in submerged_points)
...
def plot_hull_3d(
    ...
    **kwargs,
    ...
    if show_waterline_plane:
        ...
        ax.plot_surface(
            X_plane, Y_plane, Z_plane, color="cyan", alpha=waterline_alpha, label="Waterline"
        )
...
def plot_profile_with_properties(
    ...
    **kwargs,
    ...
    ax = plot_profile(
        profile=profile, waterline_z=waterline_z, heel_angle=heel_angle, ax=ax, **kwargs
    )
    ...
    if abs(heel_angle) > 1e-6:
        ...
        transformed_profile = Profile(
            station=profile.station, points=[apply_heel(pt, heel_angle) for pt in profile.points]
        )
    else:
        transformed_profile = profile
    ...
    if show_centroid and area > 1e-6:
        try:
            ...
            ax.plot(
                centroid_y, centroid_z, "rx", markersize=12, markeredgewidth=3, label="Centroid"
            )
    ...
    if show_waterline_intersection:
        try:
            ...
            if len(intersections) > 0:
                ...
                ax.plot(y_int, z_int, "go", markersize=8, label="Waterline Intersections")
                ax.legend(loc="best", fontsize=9)
...
def configure_plot_style(grid: bool = True, style: str = "seaborn-v0_8-darkgrid") -> None:
    """
...
def save_figure(
    fig: plt.Figure, filepath: Path, dpi: int = 300, bbox_inches: str = "tight", **kwargs
) -> None:
    """
            ...
            if len(positive_indices) > 0:
                ...
                if last_positive_idx < len(curve.gz_values) - 1:
                    ...
                    ax.plot(
                        ...
                        marker="^",
                        markersize=12,
                        color=vanishing_color,
                        markeredgecolor="darkorange",
                        markeredgewidth=1.5,
                        label="Vanishing Stability",
                        zorder=5,
                    )
...
def plot_gz_at_angles(
    curve, angles: List[float], ax: Optional[Axes] = None, **kwargs  # StabilityCurve type
) -> Axes:
    """
...
def plot_righting_moment_curve(
    ...
    **kwargs,
    ...
    ax.set_title(
        f"Righting Moment Curve (Displacement = {displacement_mass:.1f} kg)",
        fontsize=13,
        fontweight="bold",
    )
...
def create_stability_report_plot(
    ...
    **kwargs,
    ...
    table = ax_metrics.table(
        cellText=metrics_data, cellLoc="left", loc="center", colWidths=[0.6, 0.4]
    )
...
def interactive_heel_explorer(
    ...
    fig = plt.figure(figsize=figsize)
    gs = fig.add_gridspec(
        3, 3, hspace=0.3, wspace=0.3, left=0.08, right=0.95, top=0.93, bottom=0.15
    )
    ...
    current_line = ax_gz.axvline(
        x=initial_heel, color="red", linestyle="--", linewidth=2, label="Current Angle"
    )
    ax_gz.legend()
    ...
    def update(heel_angle):
        ...
        plot_hull_3d(
            ...
            heel_angle=heel_angle,
            ax=ax_3d,
            title=f"Hull at {heel_angle:.1f}° Heel",
        )
        ...
        plot_profile(
            ...
            heel_angle=heel_angle,
            ax=ax_profile,
            title=f"Profile at Station {mid_station:.2f}",
        )
...
{str(e)[:100]}
    ...
    slider = Slider(
        ax_slider, "Heel Angle (°)", heel_range[0], heel_range[1], valinit=initial_heel, valstep=0.5
    )
...
def interactive_stability_curve(
    ...
    fig = plt.figure(figsize=figsize)
    gs = fig.add_gridspec(
        2, 3, hspace=0.3, wspace=0.3, left=0.08, right=0.95, top=0.93, bottom=0.08
    )
    ...
    (line,) = ax_curve.plot(
        ...
        label="GZ Curve",
        picker=5,
    )  # Enable picking within 5 points
    ...
    ax_curve.set_title(
        "Stability Curve\n(Click on curve to see details)", fontsize=11, fontweight="bold"
    )
    ax_curve.legend()

    # Marker for selected point
    (selected_point,) = ax_curve.plot([], [], "ro", markersize=10, label="Selected", zorder=5)
...
def animate_heel_sequence(
    ...
    """
    from ..hydrostatics import calculate_volume, calculate_center_of_buoyancy
    from ..stability import calculate_gz
    ...
    fig = plt.figure(figsize=figsize)
    gs = fig.add_gridspec(
        3, 3, hspace=0.3, wspace=0.3, left=0.08, right=0.95, top=0.93, bottom=0.12
    )
...
Error: {str(e)[:60]}
    ...
    # Create animation
    anim = FuncAnimation(
        fig, animate, init_func=init, frames=n_frames, interval=interval, blit=False, repeat=True
    )
...
def interactive_cg_adjustment(
    ...
    # Create figure with subplots
    fig = plt.figure(figsize=figsize)
    gs = fig.add_gridspec(3, 2, hspace=0.35, wspace=0.3, left=0.1, right=0.95, top=0.88, bottom=0.2)
    ...
    ax_curve.plot(heel_angles, original_gz, "b-", linewidth=2, label="Original", alpha=0.7)
    (adjusted_line,) = ax_curve.plot([], [], "r-", linewidth=2, label="Adjusted", alpha=0.7)
    ax_curve.axhline(y=0, color="gray", linestyle="--", linewidth=1)
    ax_curve.grid(True, alpha=0.3)
    ax_curve.set_xlabel("Heel Angle (degrees)", fontsize=11)
    ax_curve.set_ylabel("GZ (m)", fontsize=11)
...
Stability Metrics:
  ...
  Vanishing: {adjusted_vanishing:.1f}° ({delta_vanishing:+.1f}°)
    ...
    slider_lcg = Slider(
        ax_slider_lcg, "LCG (m)", lcg_range[0], lcg_range[1], valinit=initial_cg.x, valstep=0.01
    )
    slider_lcg.on_changed(update)

    ax_slider_vcg = plt.axes([0.15, 0.05, 0.7, 0.03])
    slider_vcg = Slider(
        ax_slider_vcg, "VCG (m)", vcg_range[0], vcg_range[1], valinit=initial_cg.z, valstep=0.01
    )
...
def interactive_waterline_explorer(
    ...
    ax_plot.set_title("Displacement and Stability vs. Waterline", fontsize=11, fontweight="bold")

    # Add lines for current waterline
    current_line = ax_plot.axvline(
        x=initial_waterline, color="green", linestyle="--", linewidth=2, label="Current"
    )
    ...
    def update(waterline_z):
        ...
        plot_hull_3d(
            ...
            heel_angle=heel_angle,
            ax=ax_3d,
            title=f"Hull at WL Z={waterline_z:.3f}m",
        )
