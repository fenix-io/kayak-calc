...
class TestValidateMetadata:
    ...
    def test_invalid_coordinate_system(self):
        ...
        metadata = {"units": "m", "coordinate_system": "invalid_system", "water_density": 1025.0}
        is_valid, errors = validate_metadata(metadata)
        assert not is_valid
...
class TestValidateProfileData:
    ...
    def test_too_few_points(self):
        ...
        profile = {
            "station": 0.0,
            "points": [{"x": 0.0, "y": 0.0, "z": 1.0}, {"x": 0.0, "y": 1.0, "z": 0.5}],
        }
        is_valid, errors = validate_profile_data(profile)
        assert not is_valid
...
class TestDefaultMetadata:
    ...
    def test_extract_metadata_from_comments(self):
        ...
        comments = ["# Units: m", "# Water density: 1000", "# Coordinate system: centerline_origin"]
...
class TestSaveHull:
    ...
    def sample_hull(self):
        ...
        # Add two profiles
        profile1 = Profile(
            0.0, [Point3D(0.0, 0.0, 1.0), Point3D(0.0, 0.5, 0.5), Point3D(0.0, -0.5, 0.5)]
        )
        profile2 = Profile(
            1.0, [Point3D(1.0, 0.0, 1.0), Point3D(1.0, 0.5, 0.5), Point3D(1.0, -0.5, 0.5)]
        )
    ...
    def test_export_cross_sections(self, temp_dir, sample_hull):
        ...
        with open(filepath, "r") as f:
            ...
            header_idx = next(
                ...
                if line and not line[0].startswith("#") and "Station" in line[0]
            )
            header = lines[header_idx]
            data_lines = lines[header_idx + 1 :]
...
class TestReportGeneration:
    ...
    def test_generate_report_with_metadata(self, temp_dir, sample_hull, sample_cg, sample_analyzer):
        ...
        metadata = {"name": "Test Kayak", "description": "Example kayak for testing"}

        generate_complete_report(
            sample_hull, sample_cg, curve, metrics, filepath, metadata=metadata
        )
...
class TestExportIntegration:
    ...
    def test_export_waterline_comparison(self, temp_dir):
        ...
        curves = calculate_stability_at_multiple_waterlines(hull, cg, waterlines)
        ...
        with open(filepath, "r") as f:
            ...
            data_lines = [
                ...
                and line[0].replace(".", "").replace("-", "").isdigit()
            ]
