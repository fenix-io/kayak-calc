...
class TestStabilityAssessment:
    ...
    def test_missing_optional_assessment(self, missing_optional_metrics):
        ...
        na_checks = [c for c in assessment.criteria_checks 
                     if c.result == CriteriaResult.NOT_APPLICABLE]
    ...
    def test_strict_mode_assessment(self, poor_metrics):
        ...
        assert (len(assessment_strict.get_failed_checks()) >= 
                len(assessment_normal.get_failed_checks()))
    ...
    def test_critical_recommendations(self, unstable_metrics):
        ...
        has_critical = any("CRITICAL" in rec or "negative GM" in rec.lower() 
                          for rec in assessment.recommendations)
        assert has_critical
...
class TestIntegration:
    ...
    def test_workflow_with_poor_stability(self, poor_metrics):
        ...
        assert any(keyword in recommendations_text 
                  for keyword in ["lower", "cg", "beam", "hull"])
