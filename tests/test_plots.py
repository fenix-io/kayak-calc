...
class TestPlotProfile(unittest.TestCase):
    ...
    def test_plot_profile_custom_colors(self):
        ...
        ax = plot_profile(
            self.profile,
            profile_color='red',
            waterline_color='green',
            submerged_color='yellow'
        )
...
class TestStabilityCurvePlotting(unittest.TestCase):
    ...
    def test_plot_stability_curve_with_slope(self):
        ...
        ax = plot_stability_curve_with_areas(
            self.curve,
            self.metrics,
            show_slope=True
        )
    ...
    def test_plot_righting_moment_curve_custom_g(self):
        ...
        ax = plot_righting_moment_curve(
            self.curve,
            displacement_mass=100.0,
            g=10.0
        )
    ...
    def test_create_stability_report_plot(self):
        ...
        fig, axes_dict = create_stability_report_plot(
            self.curve,
            self.metrics,
            hull=self.hull
        )
...
class TestInteractiveVisualization(unittest.TestCase):
    ...
    def test_interactive_heel_explorer_heel_ranges(self):
        ...
        fig1 = interactive_heel_explorer(
            self.hull, self.cg, heel_range=(0.0, 30.0)
        )
        self.assertIsNotNone(fig1)
        plt.close(fig1)
        
        # Test large heel range
        fig2 = interactive_heel_explorer(
            self.hull, self.cg, heel_range=(0.0, 180.0)
        )
        self.assertIsNotNone(fig2)
        plt.close(fig2)
    ...
    def test_animate_heel_sequence_heel_ranges(self):
        ...
        # Small range
        fig1, anim1 = animate_heel_sequence(
            self.hull, self.cg, heel_range=(0.0, 30.0), n_frames=10
        )
        self.assertIsNotNone(fig1)
        plt.close(fig1)
        
        # Larger range
        fig2, anim2 = animate_heel_sequence(
            self.hull, self.cg, heel_range=(0.0, 90.0), n_frames=10
        )
        self.assertIsNotNone(fig2)
        plt.close(fig2)
    ...
    def test_interactive_waterline_explorer_with_parameters(self):
        ...
        fig = interactive_waterline_explorer(
            self.hull,
            self.cg,
            waterline_range=(-0.3, 0.1),
            initial_waterline=-0.1,
            heel_angle=15.0
        )
    ...
    def test_interactive_waterline_explorer_ranges(self):
        ...
        # Small range
        fig1 = interactive_waterline_explorer(
            self.hull, self.cg, waterline_range=(-0.2, 0.0)
        )
        self.assertIsNotNone(fig1)
        plt.close(fig1)
        
        # Large range
        fig2 = interactive_waterline_explorer(
            self.hull, self.cg, waterline_range=(-0.6, 0.3)
        )
        self.assertIsNotNone(fig2)
        plt.close(fig2)
    ...
    def test_interactive_waterline_explorer_heeled(self):
        ...
        fig1 = interactive_waterline_explorer(
            self.hull, self.cg, heel_angle=0.0
        )
        self.assertIsNotNone(fig1)
        plt.close(fig1)
        
        fig2 = interactive_waterline_explorer(
            self.hull, self.cg, heel_angle=30.0
        )
        self.assertIsNotNone(fig2)
        plt.close(fig2)
    ...
    def test_interactive_functions_with_minimal_hull(self):
        ...
        minimal_hull.add_profile(Profile(0.0, [
            Point3D(0.0, -0.3, 0.1),
            Point3D(0.0, -0.3, -0.3),
            Point3D(0.0, 0.3, -0.3),
            Point3D(0.0, 0.3, 0.1),
        ]))
        minimal_hull.add_profile(Profile(2.0, [
            Point3D(2.0, -0.3, 0.1),
            Point3D(2.0, -0.3, -0.3),
            Point3D(2.0, 0.3, -0.3),
            Point3D(2.0, 0.3, 0.1),
