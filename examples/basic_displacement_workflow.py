...
def main():
    ...
    ax_hull = plot_hull_3d(hull, waterline_z=waterline_z, show_waterline_plane=True)
    ...
    station_positions = np.linspace(
        stations[0], stations[-1], min(5, len(stations))  # Show up to 5 sections
    )

    # Get profiles at these stations
    profiles_to_plot = [hull.get_profile(station) for station in station_positions]

    fig_sections, ax_sections = plot_multiple_profiles(profiles_to_plot, waterline_z=waterline_z)
