...
def main():
    ...
    # Define CG
    cg = CenterOfGravity(lcg=2.5, vcg=-0.15, tcg=0.0, total_mass=100.0, num_components=1)
    ...
    stations = np.linspace(hull_stations[0], hull_stations[-1], 8)  # Show 8 sections
    ...
    fig_multi = plot_multiple_stability_curves(
        curves, labels, title="Stability Comparison: CG Height Variation", figsize=(10, 6)
    )
    ...
    if not SKIP_INTERACTIVE:
        ...
        try:
            interactive_heel_explorer(hull, cg, waterline_z=waterline_z, max_angle=70)
    ...
    if not SKIP_INTERACTIVE:
        ...
        try:
            interactive_stability_curve(hull, cg, waterline_z=waterline_z, max_angle=70)
    ...
    if not SKIP_INTERACTIVE:
        ...
        try:
            interactive_cg_adjustment(hull, cg, waterline_z=waterline_z)
    ...
    if not SKIP_INTERACTIVE:
        ...
        try:
            interactive_waterline_explorer(hull, cg, waterline_z=waterline_z)
