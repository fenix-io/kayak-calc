...
def create_realistic_kayak_hull(length: float = 4.5, max_beam: float = 0.6, num_stations: int = 15) -> KayakHull:
...
def example_2_cb_at_waterlines():
    ...
    for wl, cb in zip(waterlines, cb_curve):
        ...
        print(f"{wl:>8.3f}       {draft:>8.3f}    {cb.lcb:>8.6f}   {cb.vcb:>8.6f}   {cb.volume:>10.6f}")
...
def example_3_cb_at_heel_angles():
    ...
    for angle, cb in zip(heel_angles, cb_at_heels):
        print(f"{angle:>5.0f}      {cb.lcb:>8.6f}   {cb.vcb:>8.6f}   {cb.tcb:>8.6f}   {cb.volume:>10.6f}")
...
def example_5_convergence():
    ...
    for n_stations in station_counts:
        ...
        results.append(cb)
        print(f"{n_stations:>5}      {cb.lcb:>8.6f}   {cb.vcb:>8.6f}   {cb.tcb:>8.6f}   {cb.volume:>10.6f}")
...
def example_6_cb_movement_visualization():
    ...
    for i in range(0, len(lcbs) - 1, 3):
        ...
        if abs(dx) > 0.001 or abs(dy) > 0.001:
            axes[1, 1].arrow(lcbs[i], vcbs[i], dx * 0.7, dy * 0.7,
                           head_width=0.02, head_length=0.01, fc='purple', ec='purple')
