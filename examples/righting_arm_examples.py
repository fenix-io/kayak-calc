...
def example_3_stability_metrics():
    ...
    curve = calculate_gz_curve(
        ...
        cg=cg,
        waterline_z=-0.3,
        heel_angles=np.arange(0, 91, 2)  # 2° steps
    )
...
def example_4_compare_cg_positions():
    ...
    for label, cg in cg_configs:
        ...
        print(f"{label:<20} {metrics.max_gz:8.6f} m   {metrics.angle_of_max_gz:8.2f}°      "
...
def example_5_multiple_waterlines():
    ...
    for wl, curve in zip(waterlines, curves):
        ...
        metrics = analyze_stability(curve)
        print(f"{wl:10.2f} m      {draft:6.3f} m    {metrics.max_gz:8.6f} m   "
...
def plot_gz_curve(curve, title="GZ Curve"):
    ...
    plt.plot(curve.heel_angles[max_idx], curve.gz_values[max_idx], 
             'ro', markersize=10, label=f'Max GZ = {curve.max_gz:.3f} m @ {curve.angle_of_max_gz:.1f}°')
    
    # Shade positive stability region
    positive_mask = curve.gz_values > 0
    if np.any(positive_mask):
        plt.fill_between(curve.heel_angles, 0, curve.gz_values, 
...
def plot_comparison(curves_with_labels, title="GZ Curve Comparison"):
    ...
    for i, (label, curve) in enumerate(curves_with_labels):
        ...
        plt.plot(curve.heel_angles, curve.gz_values, 
...
def main():
    ...
    wl_curves_labeled = [
        (f"WL = {wl:.2f} m", curve) 
        for wl, curve in zip([-0.45, -0.35, -0.25, -0.15], wl_curves)
    ]
