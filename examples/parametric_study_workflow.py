...
def main():
    ...
    cg_configurations = {
        'Empty': CenterOfGravity(
            lcg=2.5, vcg=-0.02, tcg=0.0, total_mass=20.0, num_components=1
        ),
    ...
    print("  " + "="*66)
    print(f"  {'Configuration':<15} {'GM (m)':<10} {'Max GZ (m)':<12} {'Vanish (°)':<12} {'Pass':<8}")
    print("  " + "-"*66)
    ...
    best_gm_config = max(cg_results.items(), 
                         key=lambda x: x[1]['summary']['gm'])
    
    # Find configuration with best max GZ
    best_gz_config = max(cg_results.items(),
                         key=lambda x: x[1]['summary']['max_gz'])
    
    # Find configuration with best vanishing angle
    best_va_config = max(cg_results.items(),
                         key=lambda x: x[1]['summary']['vanishing_angle'])
    ...
    passing_configs = sum(1 for res in cg_results.values() 
                         if res['criteria'].overall_pass)
    ...
    if passing_configs == len(cg_results):
        ...
        passing_names = [name for name, res in cg_results.items() 
                        if res['criteria'].overall_pass]
    ...
    with open(report_file, 'w') as f:
        ...
        for config_name, results in cg_results.items():
            ...
            f.write(f"| {config_name} | {cg.total_mass:.1f} | "
