...
def example_3_loading_adjustments():
    ...
    empty_cg = create_cg_manual(lcg=2.5, vcg=0.0, tcg=0.0, total_mass=28.0)  # Hull only

    print("\n1. Empty Kayak:")
    print(
        f"   LCG: {empty_cg.lcg:.3f} m, VCG: {empty_cg.vcg:.3f} m, Mass: {empty_cg.total_mass:.1f} kg"
    )
    ...
    print("\n2. With Paddler:")
    print(
        f"   LCG: {with_paddler.lcg:.3f} m, VCG: {with_paddler.vcg:.3f} m, Mass: {with_paddler.total_mass:.1f} kg"
    )
    ...
    print("\n3. Fully Loaded:")
    print(
        f"   LCG: {fully_loaded.lcg:.3f} m, VCG: {fully_loaded.vcg:.3f} m, Mass: {fully_loaded.total_mass:.1f} kg"
    )
...
def example_5_validation_criteria():
    ...
    test_cases = [
        {"name": "Balanced loading", "cg": create_cg_manual(2.0, 0.2, 0.0, 100.0), "criteria": {}},
        {
    ...
    for case in test_cases:
        ...
        cg = case["cg"]
        print(
            f"  CG: LCG={cg.lcg:.2f} m, VCG={cg.vcg:.2f} m, TCG={cg.tcg:.2f} m, Mass={cg.total_mass:.1f} kg"
        )
...
def example_6_component_positions():
    ...
    # Plot CG
    ax1.scatter(cg.lcg, cg.vcg, s=200, marker="x", color="red", linewidths=3, label="CG", zorder=5)
    ax1.legend(loc="best")
    ...
    # Plot CG
    ax2.scatter(cg.lcg, cg.tcg, s=200, marker="x", color="red", linewidths=3, label="CG", zorder=5)
    ax2.legend(loc="best")
