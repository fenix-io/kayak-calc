...
def example_2_heel_angle_effects():
    ...
    for props in properties:
        print(
            f"{props.heel_angle:<10.1f} {props.area:<12.6f} {props.centroid_y:<16.6f} {props.centroid_z:<16.6f}"
        )
...
def example_4_visualization():
    ...
    fig.suptitle(
        "Cross-Section Properties at Different Heel Angles", fontsize=16, fontweight="bold"
    )
    ...
    for idx, (ax, heel_angle) in enumerate(zip(axes.flat, heel_angles)):
        ...
        ax.plot(
            y_range, [props.waterline_z, props.waterline_z], "c--", linewidth=1.5, label="Waterline"
        )
