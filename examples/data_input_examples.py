...
def example_2_load_from_csv():
    ...
    print(
        f"  First point: ({profile.points[0].x:.2f}, {profile.points[0].y:.2f}, {profile.points[0].z:.2f})"
    )
...
def example_4_validate_before_loading():
    ...
    if is_valid:
        ...
        load_hull_from_json("data/sample_hull_simple.json")
...
def example_5_handle_errors():
    ...
    try:
        load_hull_from_json("data/nonexistent_file.json")
