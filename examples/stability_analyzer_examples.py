...
def example1_basic_usage():
    ...
    analyzer = StabilityAnalyzer(
        hull=hull,
        cg=cg,
        waterline_z=-0.2  # Waterline at z = -0.2
    )
...
def example2_generate_stability_curve():
    ...
    plt.axvline(x=curve.angle_of_max_gz, color='r', linestyle='--', alpha=0.5, 
...
def example5_compare_waterlines():
    ...
    for label, curve, metrics in results:
        print(f"{label:<15} {curve.waterline_z:<10.2f} {metrics.max_gz:<12.4f} "
    ...
    for label, curve, metrics in results:
        plt.plot(curve.heel_angles, curve.gz_values, linewidth=2, 
...
def example6_custom_angle_range():
    ...
    curve_fine = analyzer.generate_stability_curve(
        min_angle=0,
        max_angle=30,
        angle_step=2
    )
    print(f"  Fine curve: {curve_fine.heel_angles[0]}° to {curve_fine.heel_angles[-1]}° "
          f"(step={2}°, {len(curve_fine.heel_angles)} points)")
    
    # 2. Extended range
    curve_extended = analyzer.generate_stability_curve(
        min_angle=0,
        max_angle=120,
        angle_step=10
    )
    print(f"  Extended curve: {curve_extended.heel_angles[0]}° to {curve_extended.heel_angles[-1]}° "
    ...
    titles = ['Fine Resolution (0-30°, step=2°)', 
...
def example7_quick_analysis():
    ...
    summary = quick_stability_analysis(
        hull=hull,
        cg=cg,
        waterline_z=-0.2
    )
