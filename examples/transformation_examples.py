...
def example_trim_transformation():
    ...
    for i, trim_angle in enumerate(trim_angles, 1):
        ...
        trim_desc = "Level" if trim_angle == 0 else f"Bow {'Up' if trim_angle > 0 else 'Down'} {abs(trim_angle)}°"
...
def example_waterline_intersection():
    ...
    for ax, heel_angle in zip(axes, heel_angles):
        ...
        if intersections:
            ...
            ax.plot(int_y, int_z, 'r*', markersize=15, label=f'Intersections ({len(intersections)})')
        ...
        ax.text(0.02, 0.98, f'Submerged Area: {area:.4f} m²',
