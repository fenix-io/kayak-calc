...
def example_longitudinal_interpolation():
    ...
    for i, profile in enumerate(interpolated_profiles):
        ...
        ax.plot(
            ...
            color=colors[i],
            marker=".",
            label=f"Interpolated at x={profile.station}",
            markersize=4,
        )
...
def example_bow_stern_interpolation():
    ...
    # Interpolate intermediate profiles
    tapered_profiles = interpolate_to_apex(bow_profile, bow_apex, num_intermediate_stations=6)
    ...
    # Plot apex point
    ax.scatter(
        [bow_apex.x], [bow_apex.y], [bow_apex.z], color="red", s=100, marker="*", label="Bow Apex"
    )
...
def example_complete_hull_interpolation():
    ...
    stern_points = [Point3D(0.0, -0.25, 0.15), Point3D(0.0, 0.0, 0.0), Point3D(0.0, 0.25, 0.15)]
    ...
    bow_points = [Point3D(4.0, -0.3, 0.12), Point3D(4.0, 0.0, 0.0), Point3D(4.0, 0.3, 0.12)]
    ...
    new_profiles = interpolate_multiple_profiles(original_profiles, target_stations.tolist())
