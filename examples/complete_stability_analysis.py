...
from src.stability import StabilityAnalyzer, quick_stability_assessment, CriteriaResult
...
def main():
    ...
    components = [
        {"mass": 18.0, "x": 2.5, "y": 0.0, "z": -0.05, "name": "Kayak hull"},
        {"mass": 75.0, "x": 2.4, "y": 0.0, "z": -0.20, "name": "Paddler (seated)"},
        {"mass": 5.0, "x": 2.0, "y": 0.0, "z": -0.10, "name": "Gear in cockpit"},
        {"mass": 2.0, "x": 3.5, "y": 0.0, "z": -0.05, "name": "Gear in stern hatch"},
    ]
    ...
    stability_curve = analyzer.generate_stability_curve(
        max_angle=max_heel_angle, angle_step=angle_step
    )
    ...
    max_gz_angle = summary["angle_of_max_gz"]
    vanishing_angle = summary["vanishing_angle"]
    range_positive = summary["range_positive"]
    ...
    if range_positive and len(range_positive) == 2:
        print(
            f"  ✓ Range of positive stability: {range_positive[0]:.1f}° to {range_positive[1]:.1f}°"
        )
    else:
        print(f"  ✓ Range of positive stability: N/A")
    ...
    criteria_assessment = quick_stability_assessment(metrics, stability_curve)

    overall_result = criteria_assessment.overall_result
    overall_pass = overall_result == CriteriaResult.PASS
    status_symbol = "✓" if overall_pass else "✗"
    status_text = overall_result.value

    print(f"\n  Overall Status: {status_symbol} {status_text}")

    # Show individual criteria results
    print("\n  Individual Criteria:")
    for check in criteria_assessment.criteria_checks:
        symbol = "✓" if check.result == CriteriaResult.PASS else "✗"
    ...
    ax_curve = plot_stability_curve(
        stability_curve, show_metrics=True, title="Stability Curve (GZ vs Heel Angle)"
    )

    curve_file = OUTPUT_DIR / "complete_analysis_stability_curve.png"
    ax_curve.figure.savefig(curve_file, dpi=150, bbox_inches="tight")
    print(f"    ✓ Saved: {curve_file.name}")

    # 8.4: Criteria report
    print("  Creating criteria assessment report...")
    fig_criteria, _ = create_stability_report_plot(
        stability_curve, metrics, hull=hull, title="Stability Criteria Assessment"
    )
    ...
    generate_complete_report(
        ...
        cb_upright=cb_result,
        criteria_results=criteria_assessment.criteria_checks,
        metadata={
