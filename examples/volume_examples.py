...
def create_realistic_kayak_hull(
    length: float = 4.5, beam: float = 0.6, depth: float = 0.25
) -> KayakHull:
    """
...
def example_1_simple_box_volume():
    ...
    print(
        f"{'Simpson':<20} {vol_simpson:<15.6f} {abs(vol_simpson - vol_analytical)/vol_analytical * 100:<15.4f}"
    )
    print(
        f"{'Trapezoidal':<20} {vol_trap:<15.6f} {abs(vol_trap - vol_analytical)/vol_analytical * 100:<15.4f}"
    )
...
def example_2_tapered_hull():
    ...
    print(
        f"  Difference: {abs(vol_simpson - vol_trap):.6f} m³ ({abs(vol_simpson - vol_trap)/vol_simpson * 100:.2f}%)"
    )
...
def example_5_volume_distribution():
    ...
    for i, vol_comp in enumerate(vol_components):
        if i < len(stations) - 1:
            ...
            print(
                f"{i+1:<10} {stations[i]:<12.3f} {stations[i+1]:<12.3f} {vol_comp:<15.6f} {pct:<12.2f}"
            )
...
def example_8_visualization():
    ...
    ax2.bar(
        segment_centers, vol_components, width=stations[1] - stations[0], alpha=0.7, color="green"
    )
    ...
    ax3.axhline(
        y=total_vol, color="k", linestyle="--", alpha=0.5, label=f"Total: {total_vol:.4f} m³"
    )
    ax3.legend()
