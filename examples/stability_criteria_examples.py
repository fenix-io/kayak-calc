...
def example2_compare_configurations():
    ...
    for name, cg in configs:
        ...
        print(
            f"{name:<15} {assessment.overall_result.value:<10} "
    ...
    bars3 = ax2.bar(
        x,
        failed,
        bottom=np.array(passed) + np.array(warnings),
        label="Failed",
        color="red",
        alpha=0.7,
    )
...
def example3_custom_criteria():
    ...
    for name, criteria in criteria_sets:
        ...
        print(
            f"{name:<15} {assessment.overall_result.value:<12} "
...
def example7_visual_criteria_report():
    ...
    ax1.axvline(
        x=metrics.angle_of_max_gz,
        color="r",
        linestyle="--",
        alpha=0.5,
        label=f"Max GZ at {metrics.angle_of_max_gz:.1f}°",
    )
    ax1.axvline(
        x=metrics.angle_of_vanishing_stability,
        color="orange",
        linestyle="--",
        alpha=0.5,
